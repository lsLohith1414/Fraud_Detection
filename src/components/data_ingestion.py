...
class DataIngestion:
    ...
    def load_source_tables(
        self, data_root_dir: str, source_folders: list[str]
        ...
        try:
            ...
            logger.info(f"Starting CSV discovery under root directory: {data_root_dir}")

            for folder in source_folders:
                folder_path = os.path.join(data_root_dir, folder)

                if not os.path.exists(folder_path):
                    logger.warning(f"Source folder not found, skipping: {folder_path}")
                    continue

                csv_files = glob.glob(os.path.join(folder_path, "*.csv"))

                if not csv_files:
                    logger.warning(f"No CSV files found inside folder: {folder_path}")
                    continue

                for csv_path in csv_files:
                    table_name = os.path.splitext(os.path.basename(csv_path))[0]

                    logger.info(f"Loading source table '{table_name}' from {csv_path}")
                    tables[table_name] = pd.read_csv(csv_path)

            logger.info(f"Successfully loaded {len(tables)} source tables")

            return tables

        except Exception as e:
            logger.error("Failed while loading source tables", exc_info=True)
            raise CustomException(e)

    def build_training_dataset(self, tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.error("Failed while merging relational dataset", exc_info=True)
            raise CustomException(e)
    ...
    def split_and_persist_data(self, dataframe: pd.DataFrame):
        ...
        """
        try:
            logger.info("Persisting raw combined dataset to artifact store")

            os.makedirs(self.data_ingestion_config.raw_data_dir, exist_ok=True)

            dataframe.to_csv(self.data_ingestion_config.raw_data_file_path, index=False)
            ...
            train_df.to_csv(
                self.data_ingestion_config.ingested_train_file_path, index=False
            )
            test_df.to_csv(
                self.data_ingestion_config.ingested_test_file_path, index=False
            )

            logger.info("Train-test split completed and artifacts saved successfully")

        except Exception as e:
            logger.error("Failed while splitting and saving dataset", exc_info=True)
            raise CustomException(e)
