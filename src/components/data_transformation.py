...
class DataTransformation:
    ...
    def create_new_features_from_existing_features(
        ...
        df["days_since_last_login"] = (df["Timestamp"] - df["LastLogin"]).dt.days

        df["txn_gap_minutes"] = (
            df.groupby("CustomerID")["Timestamp"].diff().dt.total_seconds().div(60)
        )
        ...
        df["user_daily_txns"] = df.groupby(["CustomerID", "date"]).cumcount()
        ...
        # ------------------------------------------------------------------
        df["is_new_location"] = df.groupby("CustomerID")["Location"].transform(
    ...
    def one_hot_encode_train_test(
        self, train_df: pd.DataFrame, test_df: pd.DataFrame, categorical_cols: List[str]
        ...
        train_ohe_df = pd.DataFrame(train_array, columns=ohe_cols, index=train_df.index)

        test_ohe_df = pd.DataFrame(test_array, columns=ohe_cols, index=test_df.index)

        train_df_encoded = pd.concat(
            [train_df.drop(columns=categorical_cols), train_ohe_df], axis=1
        )

        test_df_encoded = pd.concat(
            [test_df.drop(columns=categorical_cols), test_ohe_df], axis=1
        )
    ...
    def oversample_smoteenn(
        self, train_df: pd.DataFrame, target_column: str
    ) -> pd.DataFrame:
        """
    ...
    def build_preprocessor(self, ohe, scaler: dict, categorical_cols: list):
        """
        ...
        if scaler.get("robust_features"):
            transformers.append(
                ("robust", scaler["robust_scaler"], scaler["robust_features"])
            )

        # MinMax-scaled numerical features (already fitted)
        if scaler.get("minmax_features"):
            transformers.append(
                ("minmax", scaler["minmax_scaler"], scaler["minmax_features"])
            )
    ...
    def initiate_data_transformation(self):
        ...
        try:
            ...
            logger.info(
                f"Adding Engineered features step successfully completed [{train_df.columns.tolist()}]"
            )
            ...
            logger.info(
                f"removing useless feature step successfully completed [{train_df.columns.tolist()}]"
            )
            ...
            logger.info(
                f"Cyclic encoding step successfully completed [{train_df.columns.tolist()}]"
            )

            # 2. ohe
            logger.info("One hot encoding step started")
            categorical_cols = ["Category"]
            train_df_encoded, test_df_encoded, ohe = self.one_hot_encode_train_test(
                train_df, test_df, categorical_cols
            )
            logger.info(
                f"One hot encoding step successfully completed [{train_df_encoded.columns.tolist()}]"
            )
            ...
            MINMAX_FEATURES = ["AnomalyScore", "amount_ratio_avg", "Age"]
            ...
            logger.info("Feature scaling step successfully completed")
            ...
            test_df_scaled.to_csv(
                self.data_transformation_config.transformed_test_file_path, index=False
            )
            ...
            train_df_resampled = self.oversample_smoteenn(
                train_df_scaled, "FraudIndicator"
            )

            train_df_resampled.to_csv(
                self.data_transformation_config.transformed_train_file_path, index=False
            )
